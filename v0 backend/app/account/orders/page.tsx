...
const fetcher = async (url: string) => {
  ...
  if (res.status === 401) return { items: [] as Order[] }
  const payload = await res.json()
  if (!payload.success) return { items: [] as Order[] }
  return payload.data as { items: Order[] }
}
...
export default function OrdersPage() {
  ...
  const { data, isLoading } = useSWR<{ items: Order[] }>('/api/orders', fetcher)
